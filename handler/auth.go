...
func (s *Server) loginUser(c echo.Context) error {
	...
	accessToken, accessTokenPayload, err := s.tokenMaker.CreateToken(user.ID, user.Username, s.tokenCfg.GetAccessTokenDuration())
	if err != nil {
		return c.JSON(http.StatusBadRequest, newError(err.Error()))
	}

	refreshToken, refreshTokenPayload, err := s.tokenMaker.CreateToken(user.ID, user.Username, s.tokenCfg.GetRefreshTokenDuration())
	if err != nil {
		return c.JSON(http.StatusBadRequest, newError(err.Error()))
	}
